...
| **自动刷新** | 交易时间（北京时间 9:15-15:05 工作日）每 15 分钟自动刷新 |
...
| 优化点 | 当前状态 | 建议方案 | 预期效果 |
|--------|---------|---------|----------|
| JSON 文件体积 | industry.json ~191KB | 只保留前端需要的字段 | 体积缩减 60%+ |
| ECharts 体积 | 全量引入 echarts.min.js (~1MB) | 按需引入（K线+折线组件） | 缩减到 ~300KB |
| 首次加载 | 所有 JS 一次性加载 | 按 Tab 懒加载模块 | 首屏加载提速 50% |
| K 线缓存 | 内存缓存，刷新丢失 | IndexedDB 持久化 | 减少重复 JSONP 请求 |
| 大数据量表格 | innerHTML 全量替换 | 虚拟滚动（500+ 行时） | 避免渲染卡顿 |

### 架构优化

| 优化点 | 当前状态 | 建议方案 |
|--------|---------|---------|
| 单文件 HTML | 640 行 HTML+CSS+JS 混合 | 迁移 Vue 3 + Vite，组件化 |
| 数据格式 | 存储东方财富原始字段码 | 抓取时转语义化字段 |
| 错误降级 | JSONP 失败显示空白 | 失败时显示上次缓存 + 提示 |
| 部署速度 | GitHub Pages 部署较慢 | 考虑 Cloudflare Pages 替代 |
...
| 历史数据 | 无积累 | 每次抓取存历史快照 |
| 数据校验 | 无 | 增加完整性检查 |
| 节假日 | 只判断周末 | 接入节假日 API，避免空跑 |
| 时区 | 依赖浏览器时区转换 | 统一 UTC 计算 |
